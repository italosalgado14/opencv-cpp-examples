...
private:
    cv::VideoCapture capture; //objeto captura de video opencv
    void (*process)(cv::Mat&, cv::Mat&); //funcion callback a llamar para procesar cada frame
    FrameProcessor *frameProcessor; //puntero a la clase implementando interfaz FrameProcesor
    bool callIt; //determinar si el callback debe ser llamado
    std::string windowNameInput; //el namewindow de entrada
    std::string windowNameOutput; //nombre de la ventana de salida
    int delay; //delay entre cada frame procesado
    long fnumber; //numero de frames procesados
    long frameToStop; //frame al cual se desea parar
    bool stop; //bool para parar el procesado

    std::vector<std::string> images; //vector de nombres de imagenes usadas como entrada
    std::vector<std::string>::const_iterator itImg; //iterador vector image
    cv::VideoWriter writer; // el objeto video writer
    std::string outputFile; //nombre de salida
    int currentIndex; //el index actual para imagenes de salida
    int digits; //number of digits in output image filename
    std::string extension; //extension de las imagenes de salida
