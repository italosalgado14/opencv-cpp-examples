...
{
    // read the image
    cv::Mat image= cv::imread("/home/laila/programacion/C++/openCV/32_representingHSB/apples.png");
    if (!image.data)
        return 0;
    ...
    cv::imshow("Value",channels[2]); // display value
    cv::namedWindow("Saturation");
    cv::imshow("Saturation",channels[1]); // display saturation
    cv::namedWindow("Hue");
    cv::imshow("Hue",channels[0]); // display hue
    ...
    // read the image
    image= cv::imread("/home/laila/programacion/C++/openCV/32_representingHSB/apples.png");
    if (!image.data)
        return 0;
    ...
    cv::Mat mask;
    detectHScolor(image,
                  20, 70, // hue from 320 degrees to 20 degrees
                  70, 255, // saturation from ~0.1 to 0.65
                  mask);
    ...
    cv::imwrite("/home/laila/programacion/C++/openCV/32_representingHSB/result.png", detected);
